...
struct RestorePlanner {
    ...
    func planMonitorConfigurationChange(_ input: TopologyInput) -> TopologyPlan {
        ...
        let visibleEntries = input.visibleWorkspaceMap.compactMap { monitorId, workspaceId -> (monitor: Monitor, workspaceId: WorkspaceDescriptor.ID)? in
            guard let monitor = input.previousMonitors.first(where: { $0.id == monitorId }) else {
                return nil
            }
            return (monitor, workspaceId)
        }
        let cacheEntries = Array(input.disconnectedVisibleWorkspaceCache)
        let focusedWorkspaceId = focusedWorkspaceId(in: input.snapshot)
        let relevantWorkspaceIds = Set(visibleEntries.map(\.workspaceId)).union(cacheEntries.map(\.value))

        let previousContexts = ContiguousArray(
            input.previousMonitors.map { makeMonitorContext(for: $0, strings: &stringTable) }
        )
        let newContexts = ContiguousArray(
            input.newMonitors.map { makeMonitorContext(for: $0, strings: &stringTable) }
        )
        let rawVisibleSnapshots = ContiguousArray(
            visibleEntries.map {
                omniwm_restore_visible_workspace_snapshot(
                    workspace_id: encode(uuid: $0.workspaceId),
                    monitor_key: makeMonitorKey(for: MonitorRestoreKey(monitor: $0.monitor), strings: &stringTable)
                )
            }
        )
        var visibleNamePenalties = ContiguousArray<UInt8>()
        visibleNamePenalties.reserveCapacity(visibleEntries.count * input.newMonitors.count)
        for entry in visibleEntries {
            for monitor in input.newMonitors {
                visibleNamePenalties.append(namePenalty(lhs: entry.monitor.name, rhs: monitor.name))
            }
        }

        let rawCacheEntries = ContiguousArray(
            cacheEntries.map {
                omniwm_restore_disconnected_cache_entry(
                    workspace_id: encode(uuid: $0.value),
                    monitor_key: makeMonitorKey(for: $0.key, strings: &stringTable)
                )
            }
        )

        let orderedWorkspaceIds = relevantWorkspaceIds.sorted { $0.uuidString < $1.uuidString }
        var workspaceFacts = ContiguousArray<omniwm_restore_workspace_monitor_fact>()
        workspaceFacts.reserveCapacity(orderedWorkspaceIds.count)
        for workspaceId in orderedWorkspaceIds {
            let exists = input.workspaceExists(workspaceId)
            let homeMonitorId = exists ? input.homeMonitorId(workspaceId, input.newMonitors) : nil
            let effectiveMonitorId = exists ? input.effectiveMonitorId(workspaceId, input.newMonitors) : nil
            workspaceFacts.append(
                omniwm_restore_workspace_monitor_fact(
                    workspace_id: encode(uuid: workspaceId),
                    home_monitor_id: rawMonitorId(homeMonitorId),
                    effective_monitor_id: rawMonitorId(effectiveMonitorId),
                    workspace_exists: exists ? 1 : 0,
                    has_home_monitor_id: homeMonitorId == nil ? 0 : 1,
                    has_effective_monitor_id: effectiveMonitorId == nil ? 0 : 1
                )
            )
        }

        var rawVisibleAssignments = ContiguousArray(
            repeating: omniwm_restore_visible_assignment(
                monitor_id: 0,
                workspace_id: zeroUUID()
            ),
            count: input.newMonitors.count
        )
        var rawCacheOutputEntries = ContiguousArray(
            repeating: omniwm_restore_disconnected_cache_output_entry(
                source_kind: 0,
                source_index: 0,
                workspace_id: zeroUUID()
            ),
            count: cacheEntries.count + input.previousMonitors.count
        )
        var rawOutput = omniwm_restore_topology_output(
            visible_assignments: nil,
            visible_assignment_capacity: rawVisibleAssignments.count,
            visible_assignment_count: 0,
            disconnected_cache_entries: nil,
            disconnected_cache_capacity: rawCacheOutputEntries.count,
        ...
        let status = stringTable.bytes.withUnsafeBufferPointer { stringBuffer in
            previousContexts.withUnsafeBufferPointer { previousBuffer in
                newContexts.withUnsafeBufferPointer { newBuffer in
                    rawVisibleSnapshots.withUnsafeBufferPointer { visibleBuffer in
                        visibleNamePenalties.withUnsafeBufferPointer { penaltyBuffer in
                            rawCacheEntries.withUnsafeBufferPointer { cacheBuffer in
                                workspaceFacts.withUnsafeBufferPointer { factBuffer in
                                    rawVisibleAssignments.withUnsafeMutableBufferPointer { assignmentBuffer in
                                        rawCacheOutputEntries.withUnsafeMutableBufferPointer { cacheOutputBuffer in
                                            rawOutput.visible_assignments = assignmentBuffer.baseAddress
                                            rawOutput.disconnected_cache_entries = cacheOutputBuffer.baseAddress

                                            var rawInput = omniwm_restore_topology_input(
                                                ...
                                                disconnected_cache_entries: cacheBuffer.baseAddress,
                                                disconnected_cache_entry_count: cacheBuffer.count,
                                                ...
                                                focused_workspace_id: encode(optionalUUID: focusedWorkspaceId),
                                                interaction_monitor_id: rawMonitorId(input.interactionMonitorId),
                                                previous_interaction_monitor_id: rawMonitorId(input.previousInteractionMonitorId),
                                                has_focused_workspace_id: focusedWorkspaceId == nil ? 0 : 1,
                                                has_interaction_monitor_id: input.interactionMonitorId == nil ? 0 : 1,
                                                has_previous_interaction_monitor_id: input.previousInteractionMonitorId == nil ? 0 : 1
                                            )

                                            return omniwm_restore_plan_topology(&rawInput, &rawOutput)
                                        }
                                    }
                                }
                            }
        ...
        guard status == OMNIWM_KERNELS_STATUS_OK else {
            return TopologyPlan(
                previousMonitors: input.previousMonitors,
                newMonitors: input.newMonitors,
                visibleAssignments: [:],
                disconnectedVisibleWorkspaceCache: input.disconnectedVisibleWorkspaceCache,
        ...
        var visibleAssignments: [Monitor.ID: WorkspaceDescriptor.ID] = [:]
        visibleAssignments.reserveCapacity(rawOutput.visible_assignment_count)
        for assignment in rawVisibleAssignments.prefix(rawOutput.visible_assignment_count) {
            visibleAssignments[Monitor.ID(displayId: assignment.monitor_id)] = decode(uuid: assignment.workspace_id)
        }

        var disconnectedCache: [MonitorRestoreKey: WorkspaceDescriptor.ID] = [:]
        disconnectedCache.reserveCapacity(rawOutput.disconnected_cache_count)
        for entry in rawCacheOutputEntries.prefix(rawOutput.disconnected_cache_count) {
            let sourceIndex = Int(entry.source_index)
            let key: MonitorRestoreKey
            switch entry.source_kind {
            case UInt32(OMNIWM_RESTORE_CACHE_SOURCE_EXISTING):
                guard sourceIndex < cacheEntries.count else { continue }
                key = cacheEntries[sourceIndex].key
            case UInt32(OMNIWM_RESTORE_CACHE_SOURCE_REMOVED_MONITOR):
                guard sourceIndex < input.previousMonitors.count else { continue }
                key = MonitorRestoreKey(monitor: input.previousMonitors[sourceIndex])
            default:
                continue
            }
            disconnectedCache[key] = decode(uuid: entry.workspace_id)
        }
        ...
        return TopologyPlan(
            ...
            refreshRestoreIntents: rawOutput.refresh_restore_intents != 0,
            notes: [
                "visible_assignments=\(visibleAssignments.count)",
                "disconnected_cache=\(disconnectedCache.count)",
            ]
        )
    }
    ...
    private func focusedWorkspaceId(in snapshot: ReconcileSnapshot) -> WorkspaceDescriptor.ID? {
        guard let focusedToken = snapshot.focusedToken else {
            return nil
        }
        return snapshot.windows.first(where: { $0.token == focusedToken })?.workspaceId
    }
