...
enum AXWindowService {
    ...
    static func titlePreferFast(windowId: UInt32) -> String? {
        ...
        let title = titleLookupProviderForTests?(windowId) ?? Platform.windowTitle(windowId)
    ...
    static func fastFrame(_ window: AXWindowRef) -> CGRect? {
        ...
        guard let frame = Platform.windowBounds(UInt32(windowId(window))) else { return nil }
