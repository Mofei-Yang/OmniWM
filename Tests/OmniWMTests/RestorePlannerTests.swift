...
@Suite struct RestorePlannerTests {
    ...
    @Test func topologyPlanningCollapsesRemovedVisibleWorkspaceOntoFallbackMonitorAndCachesMigration() {
        let planner = RestorePlanner()
        let left = makeRestorePlannerTestMonitor(
            displayId: 10,
            name: "Left",
        ...
        let plan = planner.planMonitorConfigurationChange(
            .init(
                snapshot: makeRestorePlannerSnapshot(interactionMonitorId: left.id),
                previousMonitors: [left, right],
                newMonitors: [left],
                visibleWorkspaceMap: [
                    left.id: ws1,
                    right.id: ws2,
                ...
                interactionMonitorId: left.id,
                previousInteractionMonitorId: nil,
                workspaceExists: { $0 == ws1 || $0 == ws2 },
                homeMonitorId: { workspaceId, _ in
                    workspaceId == ws1 ? left.id : nil
                },
                effectiveMonitorId: { _, _ in left.id }
            )
        )

        #expect(plan.refreshRestoreIntents == true)
        #expect(plan.visibleAssignments[left.id] == ws2)
        #expect(plan.disconnectedVisibleWorkspaceCache.count == 1)
        #expect(plan.disconnectedVisibleWorkspaceCache.values.contains(ws2))
        #expect(plan.interactionMonitorId == left.id)
        #expect(plan.notes == ["visible_assignments=1", "disconnected_cache=1"])
    }

    @Test func topologyPlanningRestoresCachedWorkspaceToNewHomeMonitorAndUsesFocusedWorkspaceForInteractionFallback() {
        let planner = RestorePlanner()
        let left = makeRestorePlannerTestMonitor(
            displayId: 10,
            name: "Left",
        ...
        let oldDetached = makeRestorePlannerTestMonitor(
            displayId: 20,
            name: "Detached",
            x: 1920,
            y: 0
        ...
        let ws1 = WorkspaceDescriptor.ID()
        let ws2 = WorkspaceDescriptor.ID()
        let focusedToken = WindowToken(pid: 77, windowId: 701)

        let plan = planner.planMonitorConfigurationChange(
            .init(
                snapshot: makeRestorePlannerSnapshot(
                    focusedToken: focusedToken,
                    interactionMonitorId: Monitor.ID(displayId: 999),
                    previousInteractionMonitorId: Monitor.ID(displayId: 998),
                    windows: [makeRestorePlannerWindowSnapshot(
                        token: focusedToken,
                        workspaceId: ws2
                    )]
                ),
                previousMonitors: [left],
                newMonitors: [left, right],
                visibleWorkspaceMap: [left.id: ws1],
                disconnectedVisibleWorkspaceCache: [
                    MonitorRestoreKey(monitor: oldDetached): ws2
                ],
                interactionMonitorId: Monitor.ID(displayId: 999),
                previousInteractionMonitorId: Monitor.ID(displayId: 998),
                workspaceExists: { $0 == ws1 || $0 == ws2 },
                ...
                effectiveMonitorId: { workspaceId, _ in
                    switch workspaceId {
                    case ws1: left.id
                    case ws2: right.id
                    default: nil
        ...
        #expect(plan.visibleAssignments[left.id] == ws1)
        #expect(plan.visibleAssignments[right.id] == ws2)
        #expect(plan.disconnectedVisibleWorkspaceCache.isEmpty)
        #expect(plan.interactionMonitorId == right.id)
        #expect(plan.previousInteractionMonitorId == nil)
    }
